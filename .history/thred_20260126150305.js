...
//   }
  ...
  if (window.thredFingerprint) {
    ...
    const fpPromise = import("https://fingerprint-cloudflare-worker-thred-dev.arta-4e0.workers.dev/ZbSRAmOMrlVyljKV")
    ...
    fpPromise
      .then(async (fp) => {
        if (fp) {
          ...
          console.debug("[Thred] New FP stored:", window.thredFingerprint);
          proceedWithFingerprint(window.thredFingerprint, browserKey);
        } else {
      ...
      .catch((err) => {
        console.debug("[Thred] FP error:", err);
        proceedWithFingerprint(null, browserKey);
      });
  }
  ...
  function proceedWithFingerprint(fingerprint, browserKey) {
    ...
    console.log("[Thred] Proceeding with fingerprint:", fingerprint);
    return
