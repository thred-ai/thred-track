// thred.js (CDN version - browserKey as auth key, proxied FP, page-view tracking on arrival)
...
(function () {
    ...
    const PAGE_VIEW_ENDPOINT = `${THRED_BASE}/events/page-view`; // New dedicated endpoint
    ...
    function proceedWithFingerprint(fingerprint, browserKey) {
      ...
      fetch(`${CONFIG_ENDPOINT}?browserKey=${encodeURIComponent(browserKey)}`, {
        ...
        .then((config) => {
          ...
          // Track page-view event immediately (replaces old initial enrich)
          trackPageView(fingerprint, browserKey);
  
          // Attach form listener for lead submit
    ...
    // ────────────────────────────────────────────────
    // Page-view tracking (fires on ChatGPT arrival)
    // ────────────────────────────────────────────────
    ...
    function handleFormSubmit(event, config) {
      ...
      if (!email || !window.thredFingerprint) {
