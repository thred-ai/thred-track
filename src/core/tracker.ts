...
export class Tracker {
   ...
   * Initialize tracker.
   *
   * The identity stack (fingerprint + Radar + Vector) runs on EVERY visit —
   * the script's job is identity capture, and identities are valuable
   * regardless of how the visitor arrived. Page-view tracking (which creates
   * a backend session) remains AI-referral-only; signals themselves are
   * generated by the intent network, not the script.
   */
  async init(): Promise<void> {
    ...
    if (!aiDetected) {
      this.logger.log('Visitor not from AI — identity capture only, no page-view tracking');
      return;
    }
